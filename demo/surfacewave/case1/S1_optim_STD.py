#!/usr/bin/env python3
"""
solve surface wave inversion problem with steepest descent optimization
Usage:
...
for iterc in range(iter_start, iter_end+1, 1):
  ...
  subjob= [
          '${py3} ' + os.path.join('${optimroot}', 'STD', 'init.py')+' '+fdir+' '+str(iterc),
          '${py3} ' + os.path.join('${optimroot}', 'STD', 'descent.py')+' '+fdir
          ]
