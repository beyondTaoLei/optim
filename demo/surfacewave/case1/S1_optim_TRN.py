#!/usr/bin/env python3
"""
solve surface wave inversion problem with TRN optimization
Usage:
...
for iterc in range(iter_start, iter_end+1, 1):
  ...
  subjob= [
          ...
          '${py3} ' + os.path.join('${demoroot}', 'prob', 'my_Hv.py')+' '+fdir
          ]
  ...
  err = 'log/err.%s.it%d'%(name, iterc)
  cmd = 'bash %s >%s 2>%s'%(jobnm, out, err)
  generate_cmdfile(jobnm, [subjob[0], subjob[1]], headers)
  idx += 1
  df.loc[idx] = [iterc, 3, 0, 0, cmd, out, err]
  ## inner loop
  for icg in range(niter_max_CG):
    jobnm = os.path.join(dirjob, 'descent_cg'+str(icg)+'.bash')
    name = os.path.basename(jobnm).split('.')[0]
    out = 'log/out.%s.it%d'%(name, iterc)
    err = 'log/err.%s.it%d'%(name, iterc)
    cmd = 'bash %s >%s 2>%s'%(jobnm, out, err)
    generate_cmdfile(jobnm, [subjob[2], subjob[1]], headers)
    idx += 1
    df.loc[idx] = [iterc, 4, icg+1, 0, cmd, out, err]
