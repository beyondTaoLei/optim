...
@code 
mpicc -o tomo_ac2d tomo_ac2d.c -I ../include/ -L../lib \
-L/home/tao/seis_software/SAC/lib -lIN_OUT -lsac -lsacio -lm -lfftw3

mpirun -np 4 tomo_ac2d verbose=1 omega1=2 omega_inc=1 \
...
int main(int argc, char *argv[]){
    ...
    float dt;
    int *irecord=NULL;
    float *vec, *fac_r, *fac_i, *Mvp, *csp;
    float *sp_inc, *sp_i_inc,  *sp_adj, *sp_i_adj;
    char f_sp_inc[256], f_sp_adj[256]; 
    char f_sp_i_inc[256], f_sp_i_adj[256];
    char str1[256];
    /*MPI parameters */
    int NP, MYID;
    ...
    /* check the file size */
    if(MYID==0){
        sprintf(str1,"%s.sp.bin",finc);
        size_wfd=get_file_size(str1);
        sprintf(str1,"%s.sp.bin",fadj);
    ...
    /* wavefield space */
    sp_inc = vector(1, mynum*nfreq);
    sp_i_inc = vector(1, mynum*nfreq);
    sp_adj = vector(1, mynum*nfreq);
    sp_i_adj = vector(1, mynum*nfreq);
    ...
    /* read the wavefields */
    sprintf(f_sp_inc,"%s.sp.bin",finc);
    sprintf(f_sp_i_inc,"%s.sp.i.bin",finc);
    sprintf(f_sp_adj,"%s.sp.bin",fadj);
    sprintf(f_sp_i_adj,"%s.sp.i.bin",fadj);
    read_wfd(f_sp_inc, f_sp_i_inc, sp_inc, sp_i_inc, nfreq, num, MYID, NP);
    read_wfd(f_sp_adj, f_sp_i_adj, sp_adj, sp_i_adj, nfreq, num, MYID, NP);
    
    /*3. calculate the correlation */
    cal_kernel(mynum, nfreq, nlen, irecord, fac_r, fac_i, sp_inc, 
               sp_i_inc, sp_adj, sp_i_adj, Mvp, csp);
    
    /* write gradient vp */
    write_mod(fout_vp, csp, num, MYID, NP);
    ...
    /*  wavefield space */
    free_vector(sp_inc ,1, mynum*nfreq);
    free_vector(sp_i_inc ,1, mynum*nfreq);
    free_vector(sp_adj ,1, mynum*nfreq);
    free_vector(sp_i_adj ,1, mynum*nfreq);
