...
int main(int argc, char **argv){
    ...
    * for FWI, RTM or some other cases */
    ...
    int nt_start=5, nt_shift=1; /*nt_shift:1 for u/ux/uy; 0 for p/vx/vy */
    float  **pspa, **ua;
    float  **  pvxa, **  pvya, **  uvxa, **  uvya;
    float **gradvp, **gradrho, **enga;
    float **fullvp, **fullrho, **fulleng, btmp;
    st_acquisition  *acq =NULL, *acqx =NULL, *acqy =NULL, *acqa =NULL;
    ...
    acqa = (st_acquisition *) malloc(sizeof(st_acquisition));
...
//     if(!VERBOSE) sprintf(LOG_FILE,"/dev/null");
    ...
    nt_start = max(5, iround(TSHIFT/DT));
    ...
    memseismograms=0;//nseismograms*ntr*ns*fac2;
...
//     rec_statuses=(MPI_Status *)malloc(REQUEST_COUNT*sizeof(MPI_Status));
    ...
    pspa  =  matrix(-nd+1,NY+nd,-nd+1,NX+nd);
    ua = matrix(-nd+1,NY+nd,-nd+1,NX+nd);
    pvxa  =  matrix(-nd+1,NY+nd,-nd+1,NX+nd);
    pvya  =  matrix(-nd+1,NY+nd,-nd+1,NX+nd);
    uvxa  =  matrix(-nd+1,NY+nd,-nd+1,NX+nd);
    uvya  =  matrix(-nd+1,NY+nd,-nd+1,NX+nd);
    gradvp  =  matrix(-nd+1,NY+nd,-nd+1,NX+nd);
    gradrho =  matrix(-nd+1,NY+nd,-nd+1,NX+nd);
    enga    =  matrix(-nd+1,NY+nd,-nd+1,NX+nd);
    
    fullvp = matrix(1,NYG,1,NXG);
    fullrho = matrix(1,NYG,1,NXG);
    fulleng = matrix(1,NYG,1,NXG);
    ...
    sprintf(fsrc,"adj_shot%i.su",ishot);
    extract_current_sources(fsrc, acqa);
    
    EPRECOND=1;
    if(EPRECOND){
        M_init(-nd+1, NX+nd, -nd+1, NY+nd, eng);
        M_init(-nd+1, NX+nd, -nd+1, NY+nd, enga);
        M_init(-nd+1, NX+nd, -nd+1, NY+nd, gradvp);
        M_init(-nd+1, NX+nd, -nd+1, NY+nd, gradrho);
    }
    /*******************************************************/
    /*******************************************************/
    ...
    if (L){
        ...
        zero_fdveps_ac(-nd+1,NY+nd,-nd+1,NX+nd,pvxa,pvya,pspa,pvxp1,pvyp1,psi_sxx_x,psi_sxy_x, psi_vxx,psi_vyx,psi_syy_y,psi_sxy_y, psi_vyy,psi_vxy,psi_vxxs);
    }
    ...
    for (nt=NT;nt>=nt_start;nt--){
        ...
        if (L) {   /* viscoelastic */
                ...
                update_p_interior(gx, gy, prho, ppi, hc, pvx, pvy, psp, u, -1.0);
                update_p_interior(gx, gy, prho, ppi, hc, pvxa, pvya, pspa, ua, 1.0);
                update_p_PML(gx, gy, prho, ppi, hc, pvxa, pvya, pspa, ua, psi_vxx, psi_vyy, K_x, a_x, b_x, K_y, a_y, b_y, NX, NY, 1.0);
        }
        
        /* explosive source */
        source_boundary(psp, acq->srcpos_loc, acq->signals_loc, nt, acq->nsrc_loc);
        if(nt==NT) source_last_snap(ishot, psp, 3);
        if(nt+nt_shift<=NT) psource(pspa, acqa->srcpos_loc, acqa->signals_loc, nt+nt_shift, acqa->nsrc_loc, 1);
        
        /* Applying free surface condition */
        if ((FREE_SURF) && (POS[2]==0)) surface_acoustic_PML(1, pspa);
        ...
        exchange_p (nd, pspa, bufferlef_to_rig, bufferrig_to_lef, buffertop_to_bot, bufferbot_to_top, req_send, req_rec );
        if(MYID == 0&&nt%10==0) time_av_s_exchange+=time_count(time6,&time7);
        
        /* update of particle velocities */
        update_v_interior(gx, gy, prip, prjp, hc, pvx, pvy, uvx, uvy, psp, -1.0);
        update_v_interior(gx, gy, prip, prjp, hc, pvxa, pvya, uvxa, uvya, pspa, 1.0);
        update_v_PML(gx, gy, prip, prjp, hc, pvxa, pvya, uvxa, uvya, pspa, psi_sxx_x, psi_syy_y, K_x_half, a_x_half, b_x_half, 
                    K_y_half, a_y_half, b_y_half, NX, NY, 1.0);
        ...
        exchange_v ( nd, pvxa, pvya, bufferlef_to_rig, bufferrig_to_lef, buffertop_to_bot, bufferbot_to_top, req_send, req_rec );
...
//                 snap(FP, nt, ++nsnap, pvxa, pvya, psxx,psyy, pspa, pu, ppi, hc, ishot, wfd_f);
//             }
//         }
        if(EPRECOND){
            eprecond(eng, pvx, pvy);
            eprecond(enga, pvxa, pvya);
        }
        calc_correlation(uvx, uvy, u, pvxa, pvya, pspa, gradrho, gradvp);
        /*******************************************************/
        /* update in one step  */
    ...
    calc_gradient(gradrho, gradvp, prho, ppi);
    eprecond1(eng, eng, enga);
    matrix_cat(NXG, NYG, NX, NY, POS[1], POS[2], eng,    fulleng, MPI_COMM_WORLD);
    matrix_cat(NXG, NYG, NX, NY, POS[1], POS[2], gradvp, fullvp,  MPI_COMM_WORLD);
    matrix_cat(NXG, NYG, NX, NY, POS[1], POS[2], gradrho,fullrho, MPI_COMM_WORLD);
    if(MYID==0){
        if(EPRECOND){
            btmp=global_percentiles(EPSILON, fulleng);
            update_precond(fulleng, btmp);
            matrix_norm(1, NXG, 1, NYG, IDX, IDY, fulleng);
            apply_precond(fulleng, fullvp);
            apply_precond(fulleng, fullrho);
            sprintf(fsrc,"precond_shot%i.bin",ishot);
            savematIDXY2(fulleng, fsrc);
        }
        sprintf(fsrc,"gradvp_shot%i.bin",ishot);
        savematIDXY2(fullvp, fsrc);
        sprintf(fsrc,"gradrho_shot%i.bin",ishot);
        savematIDXY2(fullrho, fsrc);
    }
    ...
    if(acqa->nsrc_loc>0){
        free_imatrix(acqa->srcpos_loc, 1,3,1,acqa->nsrc_loc);
        free_matrix(acqa->signals_loc, 1,acqa->nsrc_loc,1,NT);
    }
    /*wavefield in freq. domain*/
    ...
    free_matrix(pspa,-nd+1,NY+nd,-nd+1,NX+nd);
    free_matrix(pvxa,-nd+1,NY+nd,-nd+1,NX+nd);
    free_matrix(pvya,-nd+1,NY+nd,-nd+1,NX+nd);
    free_matrix(ua,-nd+1,NY+nd,-nd+1,NX+nd);
    free_matrix(uvxa,-nd+1,NY+nd,-nd+1,NX+nd);
    free_matrix(uvya,-nd+1,NY+nd,-nd+1,NX+nd);
    free_matrix(eng,-nd+1,NY+nd,-nd+1,NX+nd);
    free_matrix(enga,-nd+1,NY+nd,-nd+1,NX+nd);
    free_matrix(gradvp,-nd+1,NY+nd,-nd+1,NX+nd);
    free_matrix(gradrho,-nd+1,NY+nd,-nd+1,NX+nd);
    free_matrix(fullvp, 1,NYG,1,NXG);
    free_matrix(fullrho, 1,NYG,1,NXG);
    free_matrix(fulleng, 1,NYG,1,NXG);
