...
fstep_estim = os.path.join(fdir, fiter, 'step.json')
sl_paras = eval(open(fstep_estim).read())
sl = sl_paras['optim']
# write 
fhist = os.path.join(fdir, 'history.csv')
if not os.path.exists(fhist):
  data = [[iterc, fc, misfit0, sl], ]
  df = pd.DataFrame(data, columns=['iter', 'fc', 'misfit', 'steplength'])
else:
  df = pd.read_csv(fhist)
  df = df[df['iter'] < iterc]
  df.loc[len(df.index)] = [iterc, fc, misfit0, sl]
