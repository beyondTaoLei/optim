...
for iterc in range(iter_start, iter_end+1, 1):
    ...
    ## collect current misfit---part1
    subjob = [
        '${py3} '+ os.path.join('${fwiroot}', 'sum_up_misfit.py')+' '+fdir+' test',
        'cp ' + fcost + ' ' + fcost + '_2'
        ]
    jobnm = os.path.join(dirjob, 'misfit01.lsf')
    generate_cmdfile(jobnm, subjob, queue_single, 1, wtime, headers, 0)
    if submit == 1: #run the tasks
        run_current_step([jobnm, ], iterc)
    ...
    subjob = [
        ...
        '${py3} ' + os.path.join('${fwiroot}', 'my_Hv.py')+' '+fdir
        ]
    jobnm = os.path.join(dirjob, 'descent1.lsf')
    generate_cmdfile(jobnm, subjob[:2], queue_single, 1, wtime, headers, 0)
    if submit == 1: #run the tasks
        run_current_step([jobnm, ], iterc)
    ...
    jobnm = os.path.join(dirjob, 'misfit02.lsf')
    generate_cmdfile(jobnm, subjob, queue_single, 1, wtime, headers, 0)
    if submit == 1: #run the tasks
        run_current_step([jobnm, ], iterc)
